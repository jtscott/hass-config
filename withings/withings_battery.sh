...
#Grab Battery
curl -s -c $SCRIPTPATH/cookies.txt -b $SCRIPTPATH/cookies.txt 'https://scalews.health.nokia.com/cgi-bin/association' -H 'Pragma: no-cache' -H 'Origin: https://dashboard.health.nokia.com' -H 'Accept-Encoding: gzip, deflate, br' -H 'Accept-Language: en-US,en;q=0.8' -H 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/60.0.3112.113 Safari/537.36' -H 'Content-Type: text/plain;charset=UTF-8' -H 'Accept: */*' -H 'Cache-Control: no-cache' -H 'Referer: https://dashboard.health.nokia.com/'"$CURUSER"'/timeline' -H 'Connection: keep-alive' -H 'DNT: 1' --data-binary 'enrich=t&sessionid='"$SESSIONKEY"'&action=getbyaccountid&appname=hmw&apppfm=web&appliver=19fe0fa' --compressed | jq .body.associations[0].deviceproperties.batterylvl | sed -e 's/^"//' -e 's/"$//'
