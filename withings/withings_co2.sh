...
#Authenticate
curl -s -c /home/pi/.homeassistant/withings/cookies.txt -b /home/pi/.homeassistant/withings/cookies.txt -H 'Origin: https://healthmate.withings.com' 'https://account.withings.com/connectionuser/account_login?appname=my2&appliver=a9467957&r=https%3A%2F%2Fhealthmate.withings.com%2F' --data 'email='"$USERNAME"'&password='"$PASSWORD"'&is_admin=' > /dev/null
#Grab Session
SESSIONKEY=$(cat /home/pi/.homeassistant/withings/cookies.txt | grep session | cut -f7 -d$'\t')
#Grab CO2
curl -s -b /home/pi/.homeassistant/withings/cookies.txt "https://healthmate.withings.com/index/service/v2/measure" -H "Origin: https://healthmate.withings.com" -H "Cookie: session_key="$SESSIONKEY"; current_user="$CURUSER"" --data "sessionid="$SESSIONKEY"&meastype=12"%"2C35&deviceid="$DEVICEID"&appname=my2&appliver=a9467957&apppfm=web&action=getmeashf" | jq .body.series[1].data[-1].value