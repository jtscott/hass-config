...
#Start session
curl -s -c /config/netatmo/cookies.txt -b /config/netatmo/cookies.txt https://auth.netatmo.com/en-US/access/login -H 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/48.0.2564.82 Safari/537.36 Edge/14.14359' > /dev/null
#Grab CSRF
CSRF=$(cat /config/netatmo/cookies.txt | grep csrf | cut -f7 -d$'\t')
#Authenticate
curl -s -c /config/netatmo/cookies.txt -b /config/netatmo/cookies.txt https://auth.netatmo.com/en-US/access/login -H 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/48.0.2564.82 Safari/537.36 Edge/14.14359' --data 'ci_csrf_netatmo='"$CSRF"'&mail='"$USERNAME"'&pass='"$PASSWORD"'&log_submit=LOGIN&stay_logged=accept' > /dev/null
#Grab Status
