...
#Start session
curl -s -c $SCRIPTPATH/cookies.txt -b $SCRIPTPATH/cookies.txt https://auth.netatmo.com/en-US/access/login -H 'User-Agent: curl 7.38.0 (arm-unknown-linux-gnueabihf) libcurl/7.38.0 OpenSSL/1.0.1t zlib/1.2.8 libidn/1.29 libssh2/1.4.3 librtmp/2.3' > /dev/null
#Grab CSRF
CSRF=$(cat $SCRIPTPATH/cookies.txt | grep csrf | cut -f7 -d$'\t')
#Authenticate
curl -s -c $SCRIPTPATH/cookies.txt -b $SCRIPTPATH/cookies.txt https://auth.netatmo.com/en-US/access/login -H 'User-Agent: curl 7.38.0 (arm-unknown-linux-gnueabihf) libcurl/7.38.0 OpenSSL/1.0.1t zlib/1.2.8 libidn/1.29 libssh2/1.4.3 librtmp/2.3' --data 'ci_csrf_netatmo='"$CSRF"'&mail='"$USERNAME"'&pass='"$PASSWORD"'&log_submit=LOGIN&stay_logged=accept' > /dev/null
#Grab Status
